...
from DisPre_RWR_And_GCN.KGReward_mu_train import KGReward
...
gcn_path = os.path.join(grand_path, 'useful_models', 'GMD_GCN', 'gcn_0.7976.pth.tar')
...
save_mu_path = os.path.join(father_path, "gmd_dispre_model_mu_train_save", "mu_train.txt")
# alpha
alpha = 0.3
# RWR奖励模型
...
kg_reward = KGReward(gcn_reward, rwr_reward, len(GMD_Disease) + len(GMD_Symptom), len(GMD_Disease), slot_set, device, save_path=save_mu_path)
    ...
    for l in [2e-6, 1e-6, 9e-7]:
        ...
        for epoch in range(5):  # 简化的训练循环
            ...
            if test_rate > success_rate or avg_loss < average_loss[-2] or test_rate > initial_test_rate:
                ...
                model_directory = os.path.join(father_path, "gmd_dispre_model_mu_train_save", str(timeStr), f"model_directory_epoch{epoch}_accuracy_{test_rate:.4f}_loss_{avg_loss:.4f}")
            ...
            os.makedirs(os.path.dirname(os.path.join(father_path, "gmd_dispre_model_mu_train_save", str(timeStr), 'training_metrics.json')), exist_ok=True)
            with open(os.path.join(father_path, "gmd_dispre_model_mu_train_save", str(timeStr), 'training_metrics.json'), 'w') as f:
